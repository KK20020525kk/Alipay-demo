...
public class ModuleService extends BaseService<Module,Integer> {
    ...
    public void addModule(Module module) {
        ...
        //模块名称 moduleName 统一层级下模块名称唯一
        AssertUtil.isTrue(null !=moduleMapper.queryModuleByGradeAndModeleName(grade,module.getModuleName()),"该层级下模块已存在");

        //如果是二级菜单 grade=1
        if (grade == 1) {
            //地址 url 二级菜单（garde=1） 非空
            AssertUtil.isTrue(StringUtils.isBlank(module.getUrl()),"URL不能为空");
            //地址 url 二级菜单（garde=1）统一层级不可重复
     ...
     *          id 非空 数据存在
     *        层级grade 非空 0|1|2
     *        模块名称   moduleName     非空，同一层级下模块名唯一（不包含修改记录本身）
    ...
    public void updateModule(Module module){
        ...
        //层级 grade 非空0|1|2
        Integer grade =module.getGrade();
        AssertUtil.isTrue(null==grade ||!(grade == 1||grade==0 || grade == 2),"菜单层级不合法");

        //模块名称   moduleName     非空，同一层级下模块名唯一（不包含修改记录本身）
        AssertUtil.isTrue(StringUtils.isBlank(module.getModuleName()),"模块名称不能为空");
        //通过层级模块名称查询资源对象
        temp=moduleMapper.queryModuleByGradeAndModeleName(grade,module.getModuleName());
        if (temp != null) {
    ...
    public void deleteModule(Integer id) {
        ...
        //如果存在子记录 则不可删除
        AssertUtil.isTrue(count>0, "该资源存在子记录，不可以删除嗷~~");
        ...
        if(count > 0){
            //删除指定资源ID的权限记录
            permissionMapper.deletePermissionByModeleId(id);
        }
        //设置记录无效
