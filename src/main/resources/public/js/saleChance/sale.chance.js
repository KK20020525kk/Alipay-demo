layui.use(['table', 'layer'], function () {
        ...
        , cols: [[
            ...
            , {
                field: 'devResult', title: '开发状态', align: 'center', templet: function (d) {
                    //调用函数返回格式化的结果
                    return formatDavResult(d.devResult);
                }
            }
     ...
     * @returns {string}
     */
    function formatDavResult(devResult) {
        if (devResult == 0) {
    ...
    /**
     * 监听头部工具栏
     *      格式：table.on('toolbar(lay_fifter属性值 )',function (data) {
    ...
    /*function deleteSaleChance(datas){
        /!**
         * 批量删除
         *   datas:选择的待删除记录数组
         *!/
    ...
    table.on('tool(saleChances)', function (data) {
        if (data.event == "edit") {
            ...
            layer.confirm('确定删除？', {icon: 3, title: "营销机会管理"}, function (index) {
                layer.close(index);
                $.ajax({
                    type: "post",
                    url: ctx + "/sale_chance/delete",
